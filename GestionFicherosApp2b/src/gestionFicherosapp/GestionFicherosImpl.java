...
public class GestionFicherosImpl implements GestionFicheros {
	...
	public void arriba() {
		...
		System.out.println("Arriba pulsado");
	...
	public void creaCarpeta(String arg0) throws GestionFicherosException {
		...
		if (!carpetaDeTrabajo.canWrite()) 
			throw new GestionFicherosException("Alerta. No se puede acceder a "
					+ file.getAbsolutePath() + ". No se puede escribir");
		
		//que no exista -> lanzará una excepción
		if (file.exists())
			throw new GestionFicherosException("Alerta. No se puede crear la carpeta "
					+ file.getAbsolutePath() + " porque ya existe");
		
		//crear la carpeta -> lanzará una excepción
		if(!file.mkdir())
			throw new GestionFicherosException("No se ha creado ninguna carpeta con "
					+ file.getAbsolutePath() + " como nombre");
		actualiza();
	}
	...
	public void creaFichero(String arg0) throws GestionFicherosException {
		File file = new File(carpetaDeTrabajo,arg0);
		//comprobar que se pueda escribir
		if (!carpetaDeTrabajo.canWrite()) 
			throw new GestionFicherosException("Alerta. No se puede acceder a "
					+ file.getAbsolutePath() + ". No se puede escribir");
		//que no exista
		if (file.exists())
			throw new GestionFicherosException("Alerta. No se puede crear el fichero "
					+ file.getAbsolutePath() + " porque ya existe");
		//crear el fichero
		else
			try {
	...
	public void elimina(String arg0) throws GestionFicherosException {
		...
		if (!carpetaDeTrabajo.canWrite()) 
			throw new GestionFicherosException("Alerta. No se puede acceder a "
					+ file.getAbsolutePath() + ". No tiene permiso");
		//comprobar si existe
		if (!file.exists())
			throw new GestionFicherosException("Alerta. No se puede eliminar "
					+ file.getAbsolutePath() + " porque no existe");
		//eliminar 
		if (!file.delete())
			throw new GestionFicherosException("La carpeta "
					+ file.getAbsolutePath() + " no se ha eliminado");
	...
	public void renombra(String arg0, String arg1)
		...
		File name = new File(carpetaDeTrabajo,arg1);
